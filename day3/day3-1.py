...
for idx in range(len(val1)):
  currTotalDistance += int(val1[idx][1:])
  if val1[idx][0] == 'R' or val1[idx][0] == 'L':
    if val1[idx][0] == 'R':
      newX = currX + int(val1[idx][1:])
    else:
      newX = currX - int(val1[idx][1:])
    coordsX.append((min(currX, newX), max(currX, newX), currY, currTotalDistance))
    currX = newX
  else:
    if val1[idx][0] == 'U':
      newY = currY + int(val1[idx][1:])
    else:
      newY = currY - int(val1[idx][1:])
    coordsY.append((min(currY, newY), max(currY, newY), currX, currTotalDistance))
    currY = newY

#coordsX.sort(key = lambda x: x[2])
#coordsY.sort(key = lambda x: x[2])
...
#  currTotalDistance += int(val2[idx][1:])
  if val2[idx][0] == 'R' or val2[idx][0] == 'L':
    if val2[idx][0] == 'R':
      newX = currX + int(val2[idx][1:])
    else:
      newX = currX - int(val2[idx][1:])
    for i in coordsY:
      if i[2] > min(currX, newX) and i[2] < max(currX, newX):
        # Possible match
        if currY > i[0] and currY < i[1]:
          print("i = %s currX = %s currY = %s newX = %s" % (i, currX, currY, newX))
          match.append(abs(i[2]) + abs(currY))
#          print("total distance: %s", (i[3] -  + abs()))
    currX = newX
  if val2[idx][0] == 'D' or val2[idx][0] == 'U':
    if val2[idx][0] == 'U':
      newY = currY + int(val2[idx][1:])
    else:
      newY = currY - int(val2[idx][1:])
    for i in coordsX:
      if i[2] > min(currY, newY) and i[2] < max(currY, newY):
        # Possible match
        if currX > i[0] and currX < i[1]:
          print("i = %s currX = %s currY = %s newY = %s" % (i, currX, currY, newY))
          match.append(abs(currX) + abs(i[2]))
    currY = newY
