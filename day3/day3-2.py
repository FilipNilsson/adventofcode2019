...
vectorsX = vectorsY = []
currX = currY = currTotalDistance = 0

for idx in range(len(val1)):
  dist = int(val1[idx][1:])
  currTotalDistance += dist
  if val1[idx][0] == 'R' or val1[idx][0] == 'L':
    if val1[idx][0] == 'R':
      newX = currX + dist
    else:
      newX = currX - dist
    vectorsX.append((currX, newX, currY, currTotalDistance))
    currX = newX
  else:
    if val1[idx][0] == 'U':
      newY = currY + dist
      ...
      newY = currY - dist
    vectorsY.append((currY, newY, currX, currTotalDistance))
    currY = newY
...
for idx in range(len(val2)):
  ...
  if val2[idx][0] == 'R' or val2[idx][0] == 'L':
    if val2[idx][0] == 'R':
      ...
      newX = currX - dist
    for i in vectorsY:
      if min(currX, newX) < i[2] < max(currX, newX):
        if min(i[0], i[1]) < currY < max(i[0], i[1]):
          match.append(abs(i[2]) + abs(currY))
          firstdist = i[3] - abs(i[1] - currY)
          seconddist=currTotalDistance - abs(newX - i[2])
          totalDist.append(firstdist + seconddist)
    currX = newX
  if val2[idx][0] == 'D' or val2[idx][0] == 'U':
    if val2[idx][0] == 'U':
      ...
      newY = currY - dist
    for i in vectorsX:
      if min(currY, newY) < i[2] < max(currY, newY):
        if min(i[0], i[1]) < currX < max(i[0], i[1]):
          match.append(abs(currX) + abs(i[2]))
          firstdist = i[3] - abs(i[1] - currX)
          seconddist= currTotalDistance - abs(newY - i[2])
          totalDist.append(firstdist + seconddist)
    currY = newY
...
print("distance to nearest intersection: %s" % match[0])
print("total wire length: %s" % totalDist[0])
